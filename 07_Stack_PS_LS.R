...
# Get list of PS files (search in subdirectories)
...
for (ps_path in ps_files) {
  ...
  # Search recursively in LS directory too
  ls_files <- list.files(ls_dir, pattern = paste0(site_base, "_median_filtered_LS\\.tif$"),
                         full.names = TRUE, recursive = TRUE)
  
  if (length(ls_files) == 0) {
    cat("⚠️ Skipping", site_base, "- LS composite not found.\n")
    next
  }
  
  ls_path <- ls_files[1]  # If multiple matches, take the first
  ...
  if (!compareRaster(ps, ls, extent=TRUE, rowcol=TRUE, crs=TRUE, res=TRUE, stopiffalse=FALSE)) {
    cat("⚠️ Skipping", site_base, "- raster dimensions don't match.\n")
    next
  }
  ...
  cat("✅ Stacked composite saved for", site_base, "\n")
}
